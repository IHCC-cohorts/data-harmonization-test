...
build/intermediate/%_mapping_suggestions_nlp_definition.tsv: $(MAP_SCRIPT_DIR)/mapping-suggest-nlp.py \
													templates/%.tsv $(GECKO_LEXICAL) \
													id_generation_% | build/intermediate
	python3 $< -z $(ZOOMA_DATASET) -c $(MAP_SCRIPT_CONFIG) -t templates/$*.tsv -g $(GECKO_LEXICAL) -p DEFINITION -o $@

build/intermediate/%_mapping_suggestions_zooma_definition.tsv: $(MAP_SCRIPT_DIR)/mapping-suggest-zooma.py \
													$(MAP_SCRIPT_CONFIG) templates/%.tsv \
													id_generation_% | build/intermediate
	python3 $< -c $(MAP_SCRIPT_CONFIG) -t templates/$*.tsv -p DEFINITION -o $@
